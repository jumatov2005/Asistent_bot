...
async def ask_anon_message(message: types.Message, state: FSMContext):
    lang, role = get_user_info(message.from_user.id)
...
async def send_anon_message(message: types.Message, state: FSMContext, bot: Bot):
    ...
    # DB Save
    db.add_anonymous_message(message.from_user.id, username, text_content, msg_type, file_id)
    ...
    # Notify Super Admins
    # Optimize: Get only super admins ID directly? get_admins already returns Rows
...
async def ask_change_lang(message: types.Message):
    # Lang text hardcoded in texts.py? No, used directly here for simplicity in prompt
...
async def save_feedback(message: types.Message, state: FSMContext):
    ...
    db.add_comment(message.from_user.id, role, username, text_content, msg_type, file_id)
